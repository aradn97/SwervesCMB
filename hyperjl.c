 ...
 *
 * The routine phi_recurs uses recursion relations to calculate
 * the functions, which is accurate but relatively slow.
 * CHECK ACCURACY FOR BETA < 1
 *
 * The routine phi_langer uses Langer's formula for a
...
int main()
{
  ...
  for(i=1; i<=400; i++){
    ...
    /*
    chi = (double)i*3.1415926536/400.; 
    */

    //phi1 = phi_recurs(l,K,beta,chi);
    phi2 = phi_langer(l,K,beta,chi);
