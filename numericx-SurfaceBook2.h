...
class numericx {
//	friend class cmbans7;
//	friend class doubleinflation;
//	friend class functions;
//	friend class neutrino;
////	friend class recfast;
...
void splini(int n)
{
	gg = (double *)malloc((n+5)*sizeof(double));
	gg[1]=0.0;
	for(int i=2;i<=n+3;i++)
		gg[i]=1.0/(4.0-gg[i-1]);
	}
...
void splder(double y[],double dy[],int n)
{
	...
	f = (double *)malloc((n+5)*sizeof(double));
   ...
   printf("Test - In splder");
   for(int i=2;i<=nl;i++)
   {
     f[i]=gg[i]*(3.0*(y[i+1]-y[i-1])-f[i-1]);
	//printf("%d %e %e %e\n",i,f[i],gg[i],y[i]);
   }
   printf("Test - In splder 1a");	
	dy[n]=f[n];

   printf("Test - In splder 1");
...
void spline(double x[],double y[],int n,double yp1,double ypn,double y2[])
{
   ...
   u = (double *)malloc((n+5)*sizeof(double));
